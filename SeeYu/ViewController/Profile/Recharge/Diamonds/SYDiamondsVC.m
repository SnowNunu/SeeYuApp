...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    SYGoodsModel *model = self.viewModel.datasource[indexPath.row];
    NSDictionary *params = @{@"userId":self.viewModel.services.client.currentUserId,@"goodsType":@"2",@"goodsId":model.goodsId,@"rechargeType":[NSString stringWithFormat:@"%ld",indexPath.row % 2 + 1]};
