...
#import "SYSearchFriendsViewController.h"
#import "SYCommonCell.h"


@interface SYAddFriendsViewController ()<UISearchResultsUpdating,UISearchControllerDelegate>
/// viewModel
@property (nonatomic, readwrite, strong) SYAddFriendsViewModel *viewModel;
/// searchController
@property (nonatomic, readwrite, strong) UISearchController *searchController;
/// resultController
@property (nonatomic, readwrite, strong) SYSearchFriendsViewController *resultController;
/// headerView
@property (nonatomic, readwrite, weak) SYSearchFriendsHeaderView *headerView;

/// style
@property (nonatomic, readwrite, assign) UIStatusBarStyle style;
@end

@implementation SYAddFriendsViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView dequeueReusableCellWithIdentifier:(NSString *)identifier forIndexPath:(NSIndexPath *)indexPath{
    return [SYCommonCell cellWithTableView:tableView style:UITableViewCellStyleSubtitle];
}
...
- (void)_setupSubViews{
    ...
    SYSearchFriendsHeaderView *headerView = [SYSearchFriendsHeaderView headerView];
    self.headerView = headerView;
    self.tableView.tableHeaderView = headerView;
    self.tableView.tableHeaderView.sy_height = 80.0f;
    /// binderViewModel
    [headerView bindViewModel:self.viewModel.headerViewModel];
    // Callback
    headerView.searchCallback = ^(SYSearchFriendsHeaderView *headerView) {
        @strongify(self);
    ...
    /// 配置搜索控制器
    /// 搜索结果vc
    SYSearchFriendsViewModel *viewModel = [[SYSearchFriendsViewModel alloc] initWithServices:self.viewModel.services params:nil];
    SYSearchFriendsViewController *resultController = [[SYSearchFriendsViewController alloc] initWithViewModel:viewModel];
    ...
    searchBar.barTintColor = SYColorFromHexString(@"#EFEFF4");
    searchBar.tintColor = [UIColor colorWithRed:0 green:(190 / 255.0) blue:(12 / 255.0) alpha:1];
    [searchBar setImage:SYImageNamed(@"VoiceSearchStartBtn") forSearchBarIcon:UISearchBarIconBookmark state:UIControlStateNormal];
    /// 设置代理
    ...
    bgView.layer.borderColor = SYColorFromHexString(@"#DFDFDD").CGColor;
    ...
    if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0) {
        ...
        // 屏幕截图 - 调用苹果官方框架实现磨砂效果
        UIImage *screenShot = [UIImage sy_screenShot].applyLightEffect;
