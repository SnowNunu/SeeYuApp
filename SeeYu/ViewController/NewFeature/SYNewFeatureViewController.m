...
/// viewModel
@property (nonatomic, readwrite, strong) SYNewFeatureViewModel *viewModel;
@end
...
- (void)clickStartButton {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:SYSwitchRootViewControllerNotification object:nil userInfo:@{SYSwitchRootViewControllerUserInfoKey:@(SYSwitchRootViewControllerFromTypeNewFeature)}];
}
...
- (void)_prepareScrollView {
    ...
    // 添加图像视图
    for (int i = 0; i < 4; ++i) {
        NSString *string = [NSString stringWithFormat:@"new_feature_%d",i + 1];
        UIImage *image = SYImageNamed(string);
    ...
    /// 适配 iOS11
    SYAdjustsScrollViewInsets_Never(self.scrollView);
}
...
- (UIButton *)startButton {
    if (_startButton == nil) {
        ...
        [_startButton setBackgroundImage:SYImageNamed(@"new_feature_finish_button") forState:UIControlStateNormal];
...
- (UIButton *)sharedButton {
    if (_sharedButton == nil) {
        ...
        [_sharedButton setImage:SYImageNamed(@"new_feature_share_false") forState:UIControlStateNormal];
        [_sharedButton setImage:SYImageNamed(@"new_feature_share_true") forState:UIControlStateSelected];
