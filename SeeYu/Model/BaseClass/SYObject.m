...
#import "SYObject.h"
#import "SYReflection.h"
#import "SYEXTRuntimeExtensions.h"
#import "NSError+SYModelException.h"
// Used to cache the reflection performed in +propertyKeys.
static void *SYObjectCachedPropertyKeysKey = &SYObjectCachedPropertyKeysKey;
...
#else
        if (error != NULL) {
            *error = [NSError sy_modelErrorWithException:ex];
        }
...
+ (NSSet *)propertyKeys {
    NSSet *cachedKeys = objc_getAssociatedObject(self, SYObjectCachedPropertyKeysKey);
    if (cachedKeys != nil) return cachedKeys;
    
    NSMutableSet *keys = [NSMutableSet set];
    
    [self enumeratePropertiesUsingBlock:^(objc_property_t property, BOOL *stop) {
        sy_propertyAttributes *attributes = sy_copyPropertyAttributes(property);
        @onExit {
            free(attributes);
        };
    ...
    objc_setAssociatedObject(self, SYObjectCachedPropertyKeysKey, keys, OBJC_ASSOCIATION_COPY);
...
+ (void)enumeratePropertiesUsingBlock:(void (^)(objc_property_t property, BOOL *stop))block {
    ...
    while (!stop && ![cls isEqual:SYObject.class]) {
        unsigned count = 0;
...
#pragma mark Merging
- (void)mergeValueForKey:(NSString *)key fromModel:(SYObject *)model {
    NSParameterAssert(key != nil);
    
    SEL selector = SYSelectorWithCapitalizedKeyPattern("merge", key, "FromModel:");
...
- (void)mergeValuesForKeysFromModel:(SYObject *)model {
