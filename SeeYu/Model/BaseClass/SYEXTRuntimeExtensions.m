...
sy_propertyAttributes *sy_copyPropertyAttributes (objc_property_t property) {
    ...
    sy_propertyAttributes *attributes = calloc(1, sizeof(sy_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate sy_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        switch (flag) {
            ...
            case 'C':
                attributes->memoryManagementPolicy = sy_propertyMemoryManagementPolicyCopy;
                break;
                
            case '&':
                attributes->memoryManagementPolicy = sy_propertyMemoryManagementPolicyRetain;
                break;
