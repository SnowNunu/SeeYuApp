...
+ (UIBarButtonItem *)sy_systemItemWithTitle:(NSString *)title
    ...
    ({
        ...
        textAttrs[NSFontAttributeName] = SYRegularFont(16.0f);
        ...
        item = [[UIBarButtonItem alloc] initWithImage:[SYImageNamed(imageName) imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStylePlain target:target action:selector];
        item;
    });
    return item;
}

+ (UIBarButtonItem *)sy_customItemWithTitle:(NSString *)title
                                 titleColor:(UIColor *)titleColor
                                  imageName:(NSString *)imageName
                                     target:(id)target
                                   selector:(SEL)selector
    ...
    [item setImage:[UIImage sy_imageAlwaysShowOriginalImageWithImageName:imageName] forState:UIControlStateNormal];
    [item.titleLabel setFont:SYRegularFont_16];
...
/// 返回按钮 带箭头的
+ (UIBarButtonItem *)sy_backItemWithTitle:(NSString *)title
                                imageName:(NSString *)imageName
                                   target:(id)target
                                   action:(SEL)action
{
    return [self sy_customItemWithTitle:title titleColor:nil imageName:imageName target:target selector:action contentHorizontalAlignment:UIControlContentHorizontalAlignmentLeft];
}
