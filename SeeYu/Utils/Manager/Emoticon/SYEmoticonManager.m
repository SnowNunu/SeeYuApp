...
@implementation SYEmoticonManager
...
/// 表情组
+ (NSArray<SYEmoticonGroup *> *)emoticonGroups{
    ...
    dispatch_once(&onceToken, ^{
        ...
        groups = (NSMutableArray *)[SYEmoticonGroup modelArrayWithJSON:packages];
        NSMutableDictionary *groupDic = [NSMutableDictionary new];
        for (int i = 0, max = (int)groups.count; i < max; i++) {
            SYEmoticonGroup *group = groups[i];
        ...
        for (NSString *path in additionals) {
            SYEmoticonGroup *group = groupDic[path];
            if (!group) continue;
            ...
            SYEmoticonGroup *addGroup = [SYEmoticonGroup modelWithJSON:infoJSON];
            if (addGroup.emoticons.count) {
                for (SYEmoticon *emoticon in addGroup.emoticons) {
