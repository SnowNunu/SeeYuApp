...
#import "SYConfigureManager.h"
#import "SYControllerHelper.h"
/// 显示状态的key
static NSString * const SYDebugTouchViewStatusKey = @"SYDebugTouchViewStatusKey";


@interface SYDebugTouchView ()
...
@implementation SYDebugTouchView

SYSingletonM(Instance)

- (instancetype)initWithFrame:(CGRect)frame{
    self = [super initWithFrame:frame];
    if (self) {
        self.image = SYImageNamed(@"assistivetouch");
        ...
        self.sy_x = [UIScreen mainScreen].bounds.size.width - self.sy_width - 20;
        self.sy_y = 84;
...
- (void)setHide:(BOOL)hide{
    ...
    [[NSUserDefaults standardUserDefaults] setBool:hide forKey:SYDebugTouchViewStatusKey];
...
- (BOOL)isHide{
    BOOL temp = [[NSUserDefaults standardUserDefaults] boolForKey:SYDebugTouchViewStatusKey];
...
- (void)_tapAction:(UITapGestureRecognizer *)tapGr{
    LCActionSheet *actionSheet = [LCActionSheet sheetWithTitle:nil cancelButtonTitle:@"取消" didDismiss:^(LCActionSheet * _Nonnull actionSheet, NSInteger buttonIndex) {
        ...
        if (buttonIndex == 1) {
            ...
            /// 主题
            NSString *title = nil;
            if ([SYConfigureManager applicationFormalSetting]) {
                title = [SYConfigureManager applicationAppStoreFormalSetting]? [NSString stringWithFormat:@"当前环境：%@", @"上线环境"]: [NSString stringWithFormat:@"当前环境：%@", @"预上线环境"];
            }else{
            ...
            UIAlertAction *switchToPrePublish = [UIAlertAction actionWithTitle:@"预上线环境" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [SYConfigureManager setApplicationFormalSetting:YES];
                [SYConfigureManager setApplicationAppStoreFormalSetting:NO];
                [[SYHTTPService sharedInstance] logoutUser];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    exit(0);
                });
            }];
            switchToPrePublish.textColor = SYColorFromHexString(@"#0ECCCA");
            /// 上线
            UIAlertAction *switchToPublish = [UIAlertAction actionWithTitle:@"上线环境" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [SYConfigureManager setApplicationFormalSetting:YES];
                [SYConfigureManager setApplicationAppStoreFormalSetting:YES];
                [[SYHTTPService sharedInstance] logoutUser];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    exit(0);
                });
            ...
            switchToPublish.textColor = SYColorFromHexString(@"#0ECCCA");
            /// 测试
            UIAlertAction *switchToDevelop = [UIAlertAction actionWithTitle:@"测试环境" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                [SYConfigureManager setApplicationFormalSetting:NO];
                [SYConfigureManager setApplicationAppStoreFormalSetting:NO];
                [[SYHTTPService sharedInstance] logoutUser];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    exit(0);
            ...
            switchToDevelop.textColor = SYColorFromHexString(@"#0ECCCA");
            /// 取消
            UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
            }];
            cancelAction.textColor = SYColorFromHexString(@"#8E929D");
            /// alertController
            ...
            if ([SYConfigureManager applicationFormalSetting]) {
                ...
                if ([SYConfigureManager applicationAppStoreFormalSetting]) {
                    /// 当前 ————> 上线环境
            ...
            [[SYControllerHelper currentViewController] presentViewController:alertController animated:YES completion:nil];
        }else if(buttonIndex == 2){
            /// Http/Https的切换
            NSString *title = [NSString stringWithFormat:@"当前方式：%@", [SYConfigureManager applicationUseHttps]?@"https":@"http"];
            
            [NSObject sy_showAlertViewWithTitle:title message:@"您确定要切换环境,程序将会闪退哦\n⚡️⚡️⚡️" confirmTitle:[SYConfigureManager applicationUseHttps]?@"http":@"https" cancelTitle:@"取消" confirmAction:^{
                [SYConfigureManager setApplicationUseHttps:![SYConfigureManager applicationUseHttps]];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    exit(0);
                });
            } cancelAction:NULL];
