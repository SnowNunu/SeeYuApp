...
/// 头像
@property (nonatomic, readwrite, weak) SYControl *avatarView;

/// tipsView
@property (nonatomic, readwrite, weak) SYMomentMessageTipsView *tipsView;
...
@implementation SYMomentProfileView
...
- (void)bindViewModel:(SYMomentProfileViewModel *)viewModel{
    ...
    /// 头像
    [self.avatarView.layer yy_setImageWithURL:viewModel.user.profileImageUrl placeholder:SYDefaultAvatar(SYDefaultAvatarTypeDefualt) options:YYWebImageOptionAllowInvalidSSLCertificates|YYWebImageOptionAllowBackgroundTask|YYWebImageOptionAvoidSetImage completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
...
- (void)_makeSubViewsConstraints{
    ...
    [self.avatarView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.and.height.mas_equalTo(SYMomentProfileViewAvatarViewWH);
        make.right.equalTo(self).with.offset(-SYMomentContentInnerMargin);
        make.top.equalTo(self.coverBtn.mas_bottom).with.offset(24-SYMomentProfileViewAvatarViewWH);
    }];
    
    /// 布局昵称
    /// CoderMikeHe Fixed: 这里由于布局问题，会引发不知原因的闪退
    [self.screenNameLable mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(self.coverBtn).with.offset(-SYMomentContentInnerMargin);
        make.right.equalTo(self.avatarView.mas_left).with.offset(-SYMomentContentLeftOrRightInset);
    ...
    [self.tipsView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.width.mas_equalTo(SYMomentProfileViewTipsViewWidth);
        make.height.mas_equalTo(SYMomentProfileViewTipsViewHeight);
...
//    /// 利用textLayout来设置text、font、textColor...
    ...
    /// 头像
    SYControl *avatarView = [[SYControl alloc] init];
    avatarView.layer.borderWidth = WXGlobalBottomLineHeight;
    avatarView.layer.borderColor = SYColorFromHexString(@"#D4D4D4").CGColor;
    self.avatarView = avatarView;
    avatarView.touchBlock = ^(SYControl *view, YYGestureRecognizerState state, NSSet *touches, UIEvent *event) {
        /// 跳转到用户详情
    ...
    /// tipsView
    SYMomentMessageTipsView *tipsView = [SYMomentMessageTipsView messageTipsView];
