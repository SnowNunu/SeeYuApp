...
FOUNDATION_EXTERN NSString *const SYViewModelIDKey;
/// 传递导航栏title的key：例如 导航栏的title...
FOUNDATION_EXTERN NSString *const SYViewModelTitleKey;
/// 传递数据模型的key：例如 商品模型的传递 用户模型的传递...
FOUNDATION_EXTERN NSString *const SYViewModelUtilKey;
/// 传递webView Request的key：例如 webView request...
FOUNDATION_EXTERN NSString *const SYViewModelRequestKey;

@protocol SYViewModelServices;

@interface SYVM : NSObject
...
@property (nonatomic, readonly, strong) id<SYViewModelServices> services;
