...
NSString *const SYViewModelIDKey = @"SYViewModelIDKey";
/// 传递导航栏title的key：例如 导航栏的title...
NSString *const SYViewModelTitleKey = @"SYViewModelTitleKey";
/// 传递数据模型的key：例如 商品模型的传递 用户模型的传递...
NSString *const SYViewModelUtilKey = @"SYViewModelUtilKey";
/// 传递webView Request的key：例如 webView request...
NSString *const SYViewModelRequestKey = @"SYViewModelRequestKey";

@interface SYVM ()

/// 整个应用的服务层 The `services` parameter in `-initWithServices:params` method.
@property (nonatomic, strong, readwrite) id<SYViewModelServices> services;
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone {
    SYVM *viewModel = [super allocWithZone:zone];
    @weakify(viewModel)
    [[viewModel
...
- (instancetype)initWithServices:(id<SYViewModelServices>)services params:(NSDictionary *)params {
    ...
    if (self) {
        ...
        self.title = params[SYViewModelTitleKey];
        /// 赋值
