...
@interface SYMomentItemViewModel ()
/// 赞cmd
...
@implementation SYMomentItemViewModel
- (instancetype)initWithMoment:(SYMoment *)moment{
    if (self = [super init]) {
#if 1
        /// 内容宽度
        CGFloat limitWidth = SYMomentCommentViewWidth();
        ...
        if (SYStringIsNotEmpty(moment.user.screenName)) {
            ...
            /// 传递点击事件需要的数据
            highlight.userInfo = @{SYMomentUserInfoKey:moment.user};
...
//        }].array;
        ...
        /// 位置
        if (SYStringIsNotEmpty(self.moment.location)) {
            /// 富文本
            NSMutableAttributedString *location  = [[NSMutableAttributedString alloc] initWithString:self.moment.location];
            location.yy_font = SYMomentCreatedAtFont;
            location.yy_color = SYMomentScreenNameTextColor;
            /// 高亮
            YYTextHighlight *highlight = [YYTextHighlight new];
            [highlight setBackgroundBorder:border];
            /// 传递数据
            highlight.userInfo = @{SYMomentLocationNameKey : self.moment.location};
        ...
        if (SYStringIsNotEmpty(self.moment.source)) {
            NSMutableAttributedString *source  = [[NSMutableAttributedString alloc] initWithString:self.moment.source];
            source.yy_font = SYMomentCreatedAtFont;
            source.yy_color = SYMomentCreatedAtTextColor;
            if (self.moment.sourceAllowClick > 0 && self.moment.sourceUrl.length>0) {
                /// 允许点击
                source.yy_color = SYMomentScreenNameTextColor;
                YYTextHighlight *highlight = [YYTextHighlight new];
                [highlight setBackgroundBorder:border];
                /// 传递数据
                if (self.moment.sourceUrl) highlight.userInfo = @{SYMomentLinkUrlKey : self.moment.sourceUrl};
        ...
        if(self.moment.attitudesList.count>0){
            /// 需要
            SYMomentAttitudesItemViewModel *attitudes = [[SYMomentAttitudesItemViewModel alloc] initWithMoment:moment];
            [self.dataSource addObject:attitudes];
            
        }
        if (self.moment.commentsList.count>0) {
            [self.dataSource addObjectsFromArray:[self.moment.commentsList.rac_sequence map:^SYMomentCommentItemViewModel *(SYComment * comment) {
                SYMomentCommentItemViewModel *viewModel = [[SYMomentCommentItemViewModel alloc] initWithComment:comment];
                return viewModel;
            }].array];
        }
        ...
        /// ----------- 尺寸属性 -----------
        /// 头像
        CGFloat avatarViewX = SYMomentContentLeftOrRightInset;
        CGFloat avatarViewY = SYMomentContentTopInset;
        CGFloat avatarViewW = SYMomentAvatarWH;
        CGFloat avatarViewH = SYMomentAvatarWH;
        self.avatarViewFrame = CGRectMake(avatarViewX, avatarViewY, avatarViewW, avatarViewH);
        
        /// 昵称
        CGFloat screenNameLableX = CGRectGetMaxX(self.avatarViewFrame)+SYMomentContentInnerMargin;
...
- (void)initialize{
    ...
    self.attitudeOperationCmd = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(NSNumber * input) {
        ...
        if (self.moment.attitudesStatus<=0) {
            ...
            [self.moment.attitudesList addObject:[SYHTTPService sharedInstance].currentUser];
        }else{
            /// 已点赞，则取消点赞
            ...
            /**
             CodeMikeHe👉这里要注意： 数组调用removeObject方法，底层是会挨个取出数据数组里面的`每个元素`与`传入的数据`比对isEqual 。笔者在SYUser中重写isEqual了比对规则
             /// 实现 SYUser.m
             ...
             */
            [self.moment.attitudesList removeObject:[SYHTTPService sharedInstance].currentUser];
        }
        ...
        if (self.moment.attitudesList.count>0) {
            /// 有数据
            SYMomentAttitudesItemViewModel * attitudes = self.dataSource.firstObject;
            if ([attitudes isKindOfClass:[SYMomentAttitudesItemViewModel class]]) {
                /// 修改数据 （移除/拼接）
                [attitudes.operationCmd execute:self.moment];
            }else{
                /// 插入数据到 index = 0 （创建数据）
                SYMomentAttitudesItemViewModel *atti = [[SYMomentAttitudesItemViewModel alloc] initWithMoment:self.moment];
...
- (void)_updateSubviewsFrameWithExpand:(BOOL)expand{
    ...
    /// -4 修正间距
    CGFloat contentLableY = CGRectGetMaxY(self.screenNameLableFrame)+SYMomentContentInnerMargin-4;
    ...
    CGFloat expandBtnW = SYMomentExpandButtonWidth;
...
//    self.photosViewFrame = (CGRect){{pictureViewX , pictureViewY},pictureViewSize};
    ...
    /// videoView
    self.videoViewFrame = (self.moment.type == SYMomentExtendTypeVideo) ? CGRectMake(pictureViewX, pictureViewY, SYMomentVideoViewWidth , SYMomentVideoViewHeight) : CGRectZero;
...
//    self.locationLableFrame = CGRectMake(locationLableX, locationLableY, self.locationLableLayout.textBoundingSize.width, self.locationLableLayout.textBoundingSize.height);
    ...
    /// 更多按钮
    CGFloat operationMoreBtnX = SY_SCREEN_WIDTH - SYMomentContentLeftOrRightInset - SYMomentOperationMoreBtnWH +(SYMomentOperationMoreBtnWH - 25)*.5f;
    CGFloat operationMoreBtnTopMargin = (self.locationLableFrame.size.height>0)?(SYMomentContentInnerMargin-5):0;
    CGFloat operationMoreBtnY = CGRectGetMaxY(self.locationLableFrame)+operationMoreBtnTopMargin;
    CGFloat operationMoreBtnW = SYMomentOperationMoreBtnWH;
    CGFloat operationMoreBtnH = SYMomentOperationMoreBtnWH;
...
- (void)_updateUpArrowViewFrameForOperationMoreChanged
{
    ...
    /// -5是为了适配
    CGFloat upArrowViewTopMargin = isAllowShowUpArrowView?(SYMomentContentInnerMargin-5):0;
    CGFloat upArrowViewY = CGRectGetMaxY(self.operationMoreBtnFrame)+upArrowViewTopMargin;
    CGFloat upArrowViewW = SYMomentUpArrowViewWidth;
    CGFloat upArrowViewH = isAllowShowUpArrowView?SYMomentUpArrowViewHeight:0;
...
- (NSAttributedString *)createdAt{
    ...
    NSString *timeStr =  [SYMomentHelper createdAtTimeWithSourceDate:self.moment.createdAt];
    if (!SYStringIsNotEmpty(timeStr)) return nil;
    NSMutableAttributedString *createdAtAttr = [[NSMutableAttributedString alloc] initWithString:timeStr];
    createdAtAttr.yy_font = SYMomentCreatedAtFont;
    createdAtAttr.yy_color = SYMomentCreatedAtTextColor;
...
- (CGSize)_pictureViewSizeWithPhotosCount:(NSUInteger)photosCount{
    ...
    /// 考虑屏幕尺寸为 320的情况
    CGFloat pictureViewItemWH = SYMomentPhotosViewItemWidth();
...
//        return picSize;
    ...
    /// 大于1的情况 统统显示 九宫格样式
    NSUInteger maxCols = SYMomentMaxCols(photosCount);
    ...
    // 计算尺寸
    CGFloat photosW = totalCols * pictureViewItemWH + (totalCols - 1) * SYMomentPhotosViewItemInnerMargin;
    CGFloat photosH = totalRows * pictureViewItemWH + (totalRows - 1) * SYMomentPhotosViewItemInnerMargin;
...
- (void)setAttributedTapCommand:(RACCommand *)attributedTapCommand{
    ...
    /// 遍历数据源 dataSource
    for (SYMomentContentItemViewModel *itemViewModel in self.dataSource) {
...
- (CGRect)sourceLableFrame{
    if (!SYStringIsNotEmpty(self.moment.source)) return CGRectZero;
    CGFloat sourceLableX = CGRectGetMaxX(self.createAtLableFrame)+SYMomentContentInnerMargin;
