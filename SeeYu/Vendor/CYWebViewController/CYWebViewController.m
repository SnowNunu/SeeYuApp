...
@property (nonatomic, strong) UIButton *reloadStopBtn;
...
#pragma mark - setupCustomView
- (void)setupCustomView {
    [self.webView mas_updateConstraints:^(MASConstraintMaker *make) {
        make.bottom.equalTo(self.view).offset(-44);
    }];
    UIImageView *lineImageView = [UIImageView new];
    lineImageView.backgroundColor = SYColor(153, 153, 153);
    [self.view addSubview:lineImageView];
    
    UIView *customBgView = [UIView new];
    customBgView.backgroundColor = SYColor(255, 255, 255);
    [self.view addSubview:customBgView];
    
    UIButton *backBtn = [UIButton new];
    [backBtn setBackgroundImage:[UIImage cy_backButtonIcon:SYColor(159, 105, 235)] forState:UIControlStateNormal];
    [[backBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        [self backBtnClicked:x];
    }];
    _backBtn = backBtn;
    [customBgView addSubview:backBtn];
    
    UIButton *forwardBtn = [UIButton new];
    [forwardBtn setBackgroundImage:[UIImage cy_forwardButtonIcon:SYColor(159, 105, 235)] forState:UIControlStateNormal];
    [[forwardBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        [self forwardBtnClicked:x];
    }];
    _forwardBtn = forwardBtn;
    [customBgView addSubview:forwardBtn];
    
    UIButton *reloadStopBtn = [UIButton new];
    [reloadStopBtn setBackgroundImage:[UIImage cy_refreshButtonIcon:SYColor(159, 105, 235)] forState:UIControlStateNormal];
    [[reloadStopBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(id x) {
        [self reloadAndStopBtnClicked:x];
    }];
    _reloadStopBtn = reloadStopBtn;
    [customBgView addSubview:reloadStopBtn];
    
    CGFloat margin = (SY_SCREEN_WIDTH / 3 - 15) / 2;
    [lineImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.width.equalTo(self.view);
        make.height.offset(1);
        make.bottom.equalTo(customBgView.mas_top);
    }];
    ...
    [backBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(customBgView);
        make.left.equalTo(customBgView).offset(margin);
    }];
    [forwardBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.width.height.centerY.equalTo(backBtn);
        make.centerX.equalTo(customBgView);
    }];
    [reloadStopBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(forwardBtn);
        make.width.height.offset(20);
        make.right.equalTo(customBgView).offset(-margin);
    }];
}
...
- (void)setNavigationButtons{
    ...
    if (self.forwardButton == nil) {
        self.forwardButton = [[UIBarButtonItem alloc] initWithImage:[UIImage cy_forwardButtonIcon:nil] style:UIBarButtonItemStylePlain target:self action:@selector(forwardBtnClicked:)];
    }
    if (self.reloadStopButton == nil) {
        self.reloadImg = [UIImage cy_refreshButtonIcon:nil];
        self.stopImg = [UIImage cy_stopButtonIcon:nil];
    ...
    if (self.actionButton == nil && self.showActionButton) {
        self.actionButton = [[UIBarButtonItem alloc] initWithImage:[UIImage cy_actionButtonIcon:nil] style:UIBarButtonItemStylePlain target:self action:@selector(actionBtnClicked:)];
        if (MINIMAL_UI) {
...
- (void)refreshButtonsStatus{
    ...
    self.forwardBtn.hidden = ![self.webView canGoForward];
    ...
    UIImage *image = loaded ? [UIImage cy_refreshButtonIcon:SYColor(159, 105, 235)] : [UIImage cy_stopButtonIcon:SYColor(159, 105, 235)];
    [self.reloadStopBtn setBackgroundImage:image forState:UIControlStateNormal];
}
