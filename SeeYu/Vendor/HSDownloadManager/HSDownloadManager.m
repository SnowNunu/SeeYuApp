...
// 文件的存放路径（caches）
#define HSFileFullpath(url) [SYSeeYuDocDirPath() stringByAppendingPathComponent:HSFileName(url)]
...
// 存储文件总长度的文件路径（caches）
#define HSTotalLengthFullpath [SYSeeYuDocDirPath() stringByAppendingPathComponent:@"totalLength.plist"]
...
- (void)createCacheDirectory
{
    ...
    if (![fileManager fileExistsAtPath:SYSeeYuDocDirPath()]) {
        [fileManager createDirectoryAtPath:SYSeeYuDocDirPath() withIntermediateDirectories:YES attributes:nil error:NULL];
    }
}
...
- (void)deleteAllFile
{
    ...
    if ([fileManager fileExistsAtPath:SYSeeYuDocDirPath()]) {
        // 删除沙盒中所有资源
        [fileManager removeItemAtPath:SYSeeYuDocDirPath() error:nil];
        // 删除任务
...
- (NSString*)getLocalFilePath:(NSString *)url {
